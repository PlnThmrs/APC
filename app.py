...
THEMES = ["Fonctions", "Listes", "Dictionnaires", "POO", "Fichiers et Exceptions","Personnalisé"] 
NIVEAUX = ["Débutant", "Intermédiaire", "Avancé","Personnalisé"]
...
#la clé API est appelé dans la fonction main ultérieurement, pas besoin de la commander à cet endroit
def afficher_menu(): 
...
def lancer_exercice():
    ...
    for i, theme in enumerate(THEMES):
        print(f"[{i+1}] {theme}")
    try:
        choix_theme = int(input("Votre choix (1-6) : "))
        if not choix_theme:
            print("Choix invalide")
            return None
        if choix_theme not in [1,2,3,4,5]:
            theme=input("Entrez votre thème personnalisé :")
        else:
            theme=THEMES[choix_theme-1]
    except:
        print("Choix invalide, entrez un chiffre")
        return None
    ...
    for i, niveau in enumerate(NIVEAUX):
        print(f"[{i+1}] {niveau}")
    try:
        choix_niveau = int(input("Votre choix (1-4) : "))
        if not choix_niveau:
            print("Choix invalide")
            return None
        if choix_niveau not in [1,2,3]:
            niveau=input("Entrez votre niveau personnalisé :")
        else:
    ...
    note_eleve=int(evaluation.get('score','score non disponible'))
    commentaire=evaluation.get('evaluation','Evaluation non disponible')
    print(f"Note : {note_eleve}/10")
    print(f"Nombre de points: +{note_eleve*10}")
    print(commentaire)
    print("Solution attendue :")
    print(solution) #je récupère le score, le commentaire et la solution attendue et j'affiche
    
    #Màj du profil
    um.mettre_a_jour_score(utilisateur_courant, note_eleve, niveau, theme) #j'envoie le nom, score et la consigne à la fonction mettre_a_jour_score du module user_manager
